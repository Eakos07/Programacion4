...
namespace WCF_Proyecto_BLL.BD
{
    public class Cls_BD_BLL
    {
        ...
        public bool ExecuteNonQuery(string sNombre_SP, DataTable dtParametros, ref string sMsjError)
        {
            ...
            {
                Cls_BD_DAL Obj_BD_DAL = new Cls_BD_DAL();
                Obj_BD_DAL.scadena_conexion = ConfigurationManager.ConnectionStrings[1].ToString();//trae el objeto de bases de datos que nos estamos conectando

                Obj_BD_DAL.Obj_sql_cnx = new SqlConnection(Obj_BD_DAL.scadena_conexion);

                if (Obj_BD_DAL.Obj_sql_cnx.State == ConnectionState.Closed)//comprueba el estdo de la conexion a la base de datos y si esta cerrado la abre
                {
                    Obj_BD_DAL.Obj_sql_cnx.Open();//abre la conexion 
                }
                ...
                #region Área de Agregar Parámetros
                    ...
                    if (dtParametros.Rows.Count > 0)//comprueba si el datatable viene vacio
                    {
                        foreach (DataRow dr in dtParametros.Rows)//busca los rows del datatable y los empieza a traer
                        {
                            SqlDbType dbt = SqlDbType.VarChar;

                            switch (dr[1].ToString())
                            {
                                case "1"://realiza un match entre el numero y la variable                            
                                        dbt = SqlDbType.Int;//cada parametro debe llavr un DbType
                                        break;                                
                                case "2":
                                    {
